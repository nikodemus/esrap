...
(xml-defrule whitespace (+ (or #\space #\tab #\newline))
  (:constant nil))

(xml-defrule string-char (or (not-doublequote character) (and #\\ #\")))

(xml-defrule string (and #\" (* string-char) #\")
  (:destructure (q1 string q2)
    (declare (ignore q1 q2))
    (text string)))

(xml-defrule starting-symbol (and "<" (+ (and (! ">") character)) ">")
  (:lambda (list)
    (intern (text list))))

(xml-defrule ending-symbol (and "<" (+ (and (! ">") character)) ">")
  (:lambda (list)
    (intern (text list))))

(xml-defrule sexp-element (and "<sexp>"
			       (+ (and (! "</sexp>") character))
			       "</sexp>")
  (:destructure (ss sexpres es)
    (declare (ignore es))
    (cons (intern (string-trim "<>" ss))
	  (sexp-parse 'sexp (text "(" sexpres ")")))))

(xml-defrule term-element (and starting-symbol
...
(xml-defrule element (and starting-symbol
...
(xml-defrule xml (or sexp-element element term-element))

(xml-parse 'xml "<test><i>text</i><sexp>(a 2 3)</sexp></testy>")
