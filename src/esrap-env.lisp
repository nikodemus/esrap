...
(defmacro define-esrap-env (symbol &key mainly-non-context)
  `(progn (eval-when (:compile-toplevel :load-toplevel :execute)
	  ...
	  ,(if mainly-non-context
	       `(progn (defmacro ,(symbolicate "DEFINE-" symbol "-RULE") (symbol args &body body)
			 `(,',(symbolicate "WITH-" symbol "-RULES")
			      (,',(symbolicate "WITH-" symbol "-CONTEXTS")
				  (def-nocontext-rule ,symbol ,args ,@body))))
		       (defmacro ,(symbolicate "DEFINE-C-" symbol "-RULE") (symbol args &body body)
			 `(,',(symbolicate "WITH-" symbol "-RULES")
			      (,',(symbolicate "WITH-" symbol "-CONTEXTS")
	       ...
	       `(progn (defmacro ,(symbolicate "DEFINE-" symbol "-RULE") (symbol args &body body)
			 `(,',(symbolicate "WITH-" symbol "-RULES")
			      (,',(symbolicate "WITH-" symbol "-CONTEXTS")
				  (defrule ,symbol ,args ,@body))))
		       (defmacro ,(symbolicate "DEFINE-NC-" symbol "-RULE") (symbol args &body body)
			 `(,',(symbolicate "WITH-" symbol "-RULES")
			      (,',(symbolicate "WITH-" symbol "-CONTEXTS")
				  (def-nocontext-rule ,symbol ,args ,@body))))))
